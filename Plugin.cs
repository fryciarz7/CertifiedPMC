...
{
    public string ModGuid { get; init; } = "com.fryciarz7.spt.certpmc";
    public string Name { get; init; } = "Certified PMC";
    ...
    jsonUtil, [
        new RouteAction<ProfileCreateRequestData>(
            ...
            async (
                ...
                output,
                cancellationToken
                ) => await callbacks.ModifySkillsAsync(url, info, sessionId, output, cancellationToken)
            )
        ]
    )
{ }
...
{
    public ValueTask<string> ModifySkillsAsync(string url, ProfileCreateRequestData info, MongoId sessionId, string? output, CancellationToken cancellationToken)
    {
